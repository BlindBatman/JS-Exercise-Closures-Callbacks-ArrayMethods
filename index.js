...
function processSum(numberList, callback) {
  ...
  for (var i = 0; i < numberList.length; i++) {
    tempNum = tempNum + numberList[i];
  }
  return callback(tempNum);
}
...
function firstNamesAllCaps(runners) {
  var names = runners.map(runner => runner.first_name.toUpperCase());
  return names;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  var shirtRunner = runners.filter(runner => {
...
function tallyUpDonations(runners) {
  var temp = runners.reduce((total, runner) => {
